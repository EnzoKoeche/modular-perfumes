...
DESC = {
 ...
 'pergunta': 'Pergunta do questionário, mantida pelo curador (RF-07). Pergunta respondida é desativada, não apagada.',
 ...
 'perfume': 'Perfume importado da API (RF-13). `api_id` evita duplicar na reimportação; `campos_revisados` guarda os campos que o curador alterou e que a API não sobrescreve (RN-07). `imagem_url` aponta para a imagem servida pela API, sem copiar o arquivo (RNF-19, R-02); `fixacao` e `projecao` vêm como texto da API.',
 ...
 'diretriz_agente': 'Regras e orientações do curador incluídas no prompt de sistema do agente (RF-25).',
...
out = ['# 07 — Modelo de dados (DER lógico v2)', '',
 ...
 f'Total: **{len(T)} tabelas** e **{len(R)} relacionamentos**. Este arquivo é gerado a partir da mesma definição do diagrama, então os dois não divergem.', '']
